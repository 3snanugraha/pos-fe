    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top', 'left', 'right']}>
      ...
      <View className="bg-white p-4">
        ...
        {!searchQuery && searchHistory.length > 0 && (
          <View className="mt-4">
            <Text className="text-gray-700 font-medium mb-2">Pencarian Terakhir</Text>
          ...
          <View className="mb-4">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {sortOptions.map((option, index) => (
                <Pressable
                  key={index}
                  onPress={() => handleSortChange(option.key, option.direction as 'asc' | 'desc')}
                  className={`px-4 py-2 rounded-full mr-2 ${
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <Text className="text-xs text-gray-500 mt-1">Stok: {item.stok}</Text>
              </Pressable>
            )}
            ListHeaderComponent={
